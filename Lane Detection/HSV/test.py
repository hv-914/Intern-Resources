...
class LaneDetector:
    def __init__(self, lane_width_cm=15, border_width_cm=1.5):
        ...
        self.yellow_threshold = 9  # Lower threshold for better sensitivity
        self.black_threshold = 17  # Lower threshold for better sensitivity
    ...
    def process_frame(self, frame):
        ...
        # Decision logic for direction and lane detection
        if center_black_percent > self.black_threshold:
            if left_yellow_percent > self.yellow_threshold and right_yellow_percent > self.yellow_threshold:
                ...
                # Center road found but no yellow lines
                self.lanes_detected = False
        else:
            # No black center road detected
            self.lanes_detected = False
        ...
        if current_direction != "unknown" and self.lanes_detected:
            if self.prev_directions.count(current_direction) >= 3:  # If majority of recent directions match
                self.direction = current_direction
        
        # Update direction to "no lanes" when no lanes are detected
        if not self.lanes_detected:
            self.direction = "no lanes"
    ...
    def _draw_visualization(self, frame, roi_height, left_boundary, right_boundary):
        ...
        elif self.direction == "no lanes":
            direction_color = (0, 0, 255)  # Red
            # Draw "NO LANES DETECTED" in bold red
            cv2.putText(frame, "NO LANES DETECTED", (width//2 - 160, height//2), 
...
def stream_video(stream_url):
    """Stream video from ESP32 camera"""
...
def main():
    ...
    # Create lane detector with specifications
    ...
    print("  NO LANES: Not enough yellow borders or black road detected")
    print("\nControls:")
    print("  'q' - Quit")
    print("  '+'/'-' - Adjust yellow threshold")
    print("  '['/']' - Adjust black threshold")
    print(f"\nCurrent thresholds - Yellow: {detector.yellow_threshold}%, Black: {detector.black_threshold}%")
    print("\n=== Direction Changes ===")
    
    print(f"Connecting to ESP32 camera at {stream_url}...")
    
    # Process frames from ESP32 camera stream
