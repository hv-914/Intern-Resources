...
stream_url = f"http://192.168.{bot_num}.14:81/stream"  # IP camera stream URL
port = 80
left_lane_history = []
right_lane_history = []
steering_history = []
...
# New variables for lane stability check
lane_position_history = deque(maxlen=10)  # Store last 10 lane positions
stability_threshold = 20  # Threshold for lane position variance
...
def get_frame_from_stream(session):
    """Get a single frame from the IP camera stream"""
    try:
...
def process_lane_deviation(deviation):
    if deviation < -25:
        ...
        queue_command(f'l({temp*100})')
        print(f'l{temp}')
    elif deviation > 25:
        temp = min((deviation - 25) // 10 + 1, 5)
        queue_command(f'r({temp*100})')
        print(f'r{temp}')
    else:
        queue_command('f')
        print(f'0')
...
def calculate_steering(frame, lanes):
    ...
    if left_lane and right_lane:
        ...
        queue_command('s')
        print("No lanes detected - Stopping")
        return frame_center, "No Lane Detected - Stopping", 0

    deviation = lane_center - frame_center
    
    # Add lane center to history
    lane_position_history.append(lane_center)
    
    # Check stability of lane positions
    if len(lane_position_history) >= 10:
        variance = np.var(lane_position_history)
        if variance < stability_threshold:
            ...
            queue_command('s')
            print("Lane not stable - Stopping")
            return lane_center, "Lane Not Stable - Stopping", deviation
    else:
        queue_command('s')
        print("Not enough data - Stopping")
        return lane_center, "Not Enough Data - Stopping", deviation
...
def main():
    ...
    while running:
        ret, frame = get_frame_from_stream(session)
        if not ret:
            print("Error getting frame from stream")
            time.sleep(1)
            continue
